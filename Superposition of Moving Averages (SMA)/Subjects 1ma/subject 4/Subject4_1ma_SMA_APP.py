...
FULLDATA_contents = sio.loadmat('SUBJECT4_1MA_TO_PYTHON.mat')  # FULLDATA_contents is an array of structures
                                                                #contains 1 structure SUBJECT4_1MA


# ALOCATE CORRECT STRUCTURES WITHIN DATA
SUBJECT4_struct = FULLDATA_contents['SUBJECT4_1MA']  #contains EEG+Tacs values and also sham




# CONVERT TO NPARRAY VARIABLES
SUBJECT4 = SUBJECT4_struct[0, 0]
...
SUBJECT4_FREE_array = SMA_ERP(SUBJECT4['DATA_SUBJECT_4'],5,500)           #apply SMA TO  DATA
SUBJECT4_SHAM_FREE_ARRAY = SMA_ERP(SUBJECT4['SHAM_SUBJECT_4'],5,500)      #apply SMA TO  SHAM DATA


#print(TEST1_FREE_array)


SUBJECT4_SMA_low = signal.filtfilt(d, c, SUBJECT4_FREE_array, padlen=3*(max(len(d),len(c))-1))   #FILTER TEST1 DATA AFTER SMA APPLIED
SUBJECT4_SMA = signal.filtfilt(b, a, SUBJECT4_SMA_low, padlen=3*(max(len(b),len(a))-1))

#print(ERP_TEST1_SMA)


SHAM_SMA_low = signal.filtfilt(d, c, SUBJECT4_SHAM_FREE_ARRAY, padlen=3*(max(len(d),len(c))-1))           #FILTER RAW SHAM DATA TO COMPARE TO RESULTS
SHAM_SMA = signal.filtfilt(b, a, SHAM_SMA_low, padlen=3*(max(len(b),len(a))-1))

#print(SHAM_SMA)

plot_t = np.arange(0,20000/500,1/500)

fig = plt.figure();
ax = fig.add_subplot(1, 1, 1)
ax.plot(plot_t,SHAM_SMA[0,:]/1000, '-k', label='Sham')
ax.plot(plot_t,SUBJECT4_SMA[0,:]/1000, '--r', label='Subject 4 Data')
ax.set_title('Subject 4 artifact free EEG and SHAM')
ax.set_ylim([-200, 200])
ax.set_xlim([0, 39])
...
sio.savemat('SUBJECT4_DATA_PYTHON.mat', {'SUBJECT4_DATA_PYTHON': SUBJECT4_SMA})
sio.savemat('SUBJECT4_SHAM_PYTHON.mat', {'SUBJECT4_SHAM_PYTHON': SHAM_SMA})
