...
FULLDATA_contents = sio.loadmat('SUBJECT2_1MA_TO_PYTHON.mat')  # FULLDATA_contents is an array of structures
                                                                #contains 1 structure SUBJECT4_1MA


# ALOCATE CORRECT STRUCTURES WITHIN DATA
SUBJECT2_struct = FULLDATA_contents['SUBJECT2_1MA']  #contains EEG+Tacs values and also sham




# CONVERT TO NPARRAY VARIABLES
SUBJECT2 = SUBJECT2_struct[0, 0]
...
SUBJECT2_FREE_array = SMA_ERP(SUBJECT2['DATA_SUBJECT_2'],5,500)           #apply SMA TO  DATA
SUBJECT2_SHAM_FREE_ARRAY = SMA_ERP(SUBJECT2['SHAM_SUBJECT_2'],5,500)      #apply SMA TO  SHAM DATA



print(SUBJECT2['DATA_SUBJECT_2'])


SUBJECT2_SMA_low = signal.filtfilt(d, c, SUBJECT2_FREE_array, padlen=3*(max(len(d),len(c))-1))   #FILTER TEST1 DATA AFTER SMA APPLIED
SUBJECT2_SMA = signal.filtfilt(b, a, SUBJECT2_SMA_low, padlen=3*(max(len(b),len(a))-1))

#print(ERP_TEST1_SMA)


SHAM_SMA_low = signal.filtfilt(d, c, SUBJECT2_SHAM_FREE_ARRAY, padlen=3*(max(len(d),len(c))-1))           #FILTER RAW SHAM DATA TO COMPARE TO RESULTS
...
ax.plot(plot_t,SUBJECT2_SMA[0,:]/1000, '-r', label='Subject 2 Data')
ax.set_title('Subject 2 artifact free EEG and SHAM')
ax.set_xlim([10, 16])
...
sio.savemat('SUBJECT2_DATA_PYTHON.mat', {'SUBJECT2_DATA_PYTHON': SUBJECT2_SMA})
sio.savemat('SUBJECT2_SHAM_PYTHON.mat', {'SUBJECT2_SHAM_PYTHON': SHAM_SMA})
