...
def CEEMDAN_VER1(s,MAX_ITERATION_NUMBER, NR, Nstd, MAX_NUMBER_IMFS = np.inf):
    ...
    while es_imf < MAX_NUMBER_IMFS:  # es_imf will be one when medias has no extrema
        for i in range(NR):
...
FULLDATA_contents = sio.loadmat('Test_1mA_5Hz_ALPHA_PH.mat')


SUBJECT1_struct = FULLDATA_contents['EEG']
...
s = s[0, tacs_t1:tacs_t2]


#final_modes = CEEMDAN_VER1(s, 50, 100, 0.02, 5)
cProfile.run('CEEMDAN_VER1(s,50,100,0.02, 6)')

#sio.savemat('PYTHON_ALPHA_250uA_40HZ_100I.mat', {'PYTHON_ALPHA_250uA_40HZ_100I': final_modes})
