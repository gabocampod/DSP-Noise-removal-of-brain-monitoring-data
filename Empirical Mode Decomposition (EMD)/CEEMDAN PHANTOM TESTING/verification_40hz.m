...
sham_tacs_t1 = 15062;   %%CHANGE sham boundary
sham_tacs_t2 = sham_tacs_t1 + 29999;
ERP_Sham_SMA=filtfilt(b,a,filtfilt(d,c,EEG.Data(1, sham_tacs_t1:sham_tacs_t2)));

load('Test_250uA_40Hz_ALPHA_PH.mat') %%CHANGE data !!!!!!!!!!
tacs_t1 = 15030+1;        %%CHANGE data boundary !!!!!!!!!!
tacs_t2 = tacs_t1 + 29999; 

RAW_DATA = EEG.Data(1,tacs_t1:tacs_t2);
RAW_DATA_FILTERED = filtfilt(b,a,filtfilt(d,c,RAW_DATA));

load('PYTHON_ALPHA_250uA_40HZ_100I.mat')    %%CHANGE PYTHON RESULTS

modes = PYTHON_ALPHA_250uA_40HZ_100I;
...
modes_original_1 = modes(1,:);

[n,o] = butter(3,[39.8/250 40.2/250],'stop');
modes(2,:) = filtfilt(n,o, modes(2,:));
modes(1,:) = filtfilt(n,o, modes(1,:));

FREE_DATA = RAW_DATA - modes_original_2(1,:) + modes(2,:);
FREE_DATA = FREE_DATA - modes_original_1(1,:) + modes(1,:);
...
Fs = 500;                                   
L = 20000;             
Y = fft(ERP_Sham_SMA(1, lower:upper));      %SHAM
P2 = abs(Y/L);
P1 = P2(1:L/2+1);
P1(2:end-1) = 2*P1(2:end-1);
Y = fft(FILTERED_FREE(1, lower:upper));    %%FILTERED_FREE
P22 = abs(Y/L);
P12 = P22(1:L/2+1);
...
hold on 
plot(f, P12, 'r')
title('SHAM vs FREE frequancy domain')
xlabel('f(Hz)')
ylabel('|P1(f)|')
...
title('SHAM vs FREE time domain')
