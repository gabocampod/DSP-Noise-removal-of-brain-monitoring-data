...
load('Test_250uA_5Hz_ALPHA_PH.mat') %%CHANGE data !!!!!!!!!!
tacs_t1 = 15063+1;        %%CHANGE data boundary !!!!!!!!!!
tacs_t2 = tacs_t1 + 29999; 

RAW_DATA = EEG.Data(1,tacs_t1:tacs_t2);

load('PYTHON_ALPHA_250uA_5HZ_100I.mat') %%CHANGE CEEMDAN RESULTS

modes = PYTHON_ALPHA_250uA_5HZ_100I;  %%change
...
Y = fft(ERP_Sham_SMA(1, lower:upper));
P2 = abs(Y/L);
P1 = P2(1:L/2+1);
P1(2:end-1) = 2*P1(2:end-1);

Y = fft(FILTERED_FREE(1, lower:upper));
P22 = abs(Y/L);
P12 = P22(1:L/2+1);
...
hold on
plot(f, P12, 'r');
title('SHAM (BLUE) vs FREE OF ARTIFACT(RED)')
xlabel('f(Hz)')
ylabel('|P1(f)|')
