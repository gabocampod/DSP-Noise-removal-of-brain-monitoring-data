...
#print(modes_white_noise[0][0][3])
#print(len(modes_white_noise[0][0]))
bug_modes_white_noise = np.zeros(shape=(2,s.size))
bug_modes_white_noise_2 = np.zeros(shape=(2,s.size))
...
for i in range(NR):
    for x in range(len(modes_white_noise[i][0])):
        #bug_modes_white_noise[0,x] = modes_white_noise[i][0][x]
        modes_white_noise_NSTD[i][0][x] = modes_white_noise[i][0][x]*Nstd  #multiply all values of first IMF by Nstd
        #bug_modes_white_noise_2[0,x] = modes_white_noise_NSTD[i][0][x]
    #print(bug_modes_white_noise_2[0]/bug_modes_white_noise[0])             #should be all Nstd value
...
while es_imf > 1:     #es_imf will be one when medias has no extrema
    for i in range(NR):
        ...
        if tamanio > k+1:
            ...
            temp = EMD_VER1(medias[-1], MAX_ITERATION_NUMBER, 1)
            temp = temp[-1][:]
        aux = aux + temp/NR
...
modes_final = [[x*stand_deviation_s for x in y] for y in modes]    #modes = modes*desvio x modes is 2d array
